...
const FavoritesList: React.FC<FavoritesListProps> = ({ tracks, favorites, onSelectTrack, onToggleFavorite, currentTrackId, isPlaying, searchQuery, totalTracks }) => {
  if (totalTracks === 0) {
    return (
  ...
  if (tracks.length === 0 && searchQuery) {
     return (
      <div className="flex flex-col items-center justify-center h-full text-gray-500/80 min-h-[352px]">
        ...
        <p className="mt-2 text-gray-500">No favorites match "{searchQuery}".</p>
      </div>
    );
  }
