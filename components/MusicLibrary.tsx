...
const MusicLibrary: React.FC<MusicLibraryProps> = ({ tracks, favorites, onSelectTrack, onToggleFavorite, currentTrackId, isPlaying, searchQuery, totalTracks }) => {
  if (totalTracks === 0) {
    return (
  ...
  if (tracks.length === 0 && searchQuery) {
     return (
      <div className="flex flex-col items-center justify-center h-full text-gray-500/80 min-h-[440px]">
        <h2 className="text-2xl font-semibold text-gray-400">No Results Found</h2>
        <p className="mt-2 text-gray-500">Try a different search for "{searchQuery}".</p>
      </div>
    );
  }
